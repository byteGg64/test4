...
<script>
  ...
  export default {
    ...
    methods: {
      /**
       * 下载歌曲之前必须要先点击歌曲，点击歌曲会调用toPlay方法，在这个方法里面会调用setUrl方法，这个时候会把url设置为song
       */
      downloadMusic() {
      ...
      /*RequestUserSongList(){
        if (this.loginIn){
          HttpManager.getCollectionOfUser(this.userId)
            .then(res => {
              this.curUserSongCollect = res;
              console.log(this.curUserSongCollect,"==============================")
            }).catch(err => {
            console.log(err)
          })
        }
